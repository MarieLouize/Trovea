...
import { buildScheduleConfig, getSlotHours, decodeSlot, isSlotAvailable } from '@/lib/utils/host';
...
/** Returns the next N working days (per schedule) starting from tomorrow */
function getNextWorkingDays(count: number, scheduleDays: number[]): Date[] {
  const days: Date[] = [];
...
function formatSlotTime(hour: number, minute: number): string {
  const totalH = hour + Math.floor(minute / 60);
  const m = minute % 60;
...
export default function TerminalPage() {
  ...
  const scheduleConfig = useMemo(
    () => merchant ? buildScheduleConfig(merchant) : undefined,
    [merchant],
  );

  const weekdays = useMemo(
    () => scheduleConfig ? getNextWorkingDays(5, scheduleConfig.days) : getNextWorkingDays(5, [1, 2, 3, 4, 5]),
    [scheduleConfig],
  );
  ...
  const handleHostSlotSelect = (date: Date, hour: number, minute = 0) => {
    if (!hostService) return;
    const dateStr = formatWeekdayShort(date);
    const timeStr = formatSlotTime(hour, minute);
  ...
  const triggerCeremony = async () => {
    ...
    const mockReceipt = {
      ...
      delivery_status: st.isDigital
        ? (manualDelivery ? 'manual_pending' : 'sent')
        : baseReceipt.delivery_status,
    };
                  ...
                  <>
                    {vendorWindow && merchant ? (
                      <>
                        ...
                        {(() => {
                          // Compute sold count per product in this window
                          const winStart = new Date(vendorWindow.opens_at).getTime();
                          const winEnd = new Date(vendorWindow.closes_at).getTime();
                              ...
                              {menuItems.map((product) => {
                                ...
                                return (
                                  <m.div
                                    key={product.id}
                                    role="listitem"
                                    className={`${styles.inventoryCard} ${isSoldOut ? styles.inventoryCardSoldOut : ''}`}
                                    whileTap={isSoldOut ? {} : { scale: 0.94 }}
                                    onClick={() => { if (!isSoldOut) handleProductTap(product); }}
                                    tabIndex={isSoldOut ? -1 : 0}
                                    aria-label={`${product.name}, ${formatCurrencyFull(product.price)}${isSoldOut ? ', sold out' : ''}`}
                                    aria-disabled={isSoldOut}
                                    onKeyDown={(e) => { if (e.key === 'Enter' && !isSoldOut) handleProductTap(product); }}
                                  >
                                    <div className={styles.inventoryThumb} aria-hidden="true">
                                      {product.images[0] ? (
                                        <img src={product.images[0]} alt={product.name} loading="lazy" />
                                      ...
                                      {isSoldOut && <div className={styles.inventoryCapOut}>Cap reached</div>}
                                    </div>
                                    <div className={styles.inventoryBody}>
                                      <div className={styles.inventoryName}>{product.name}</div>
                                      {product.description && (
                                      ...
                                      <div className={styles.inventoryPrice}>{formatCurrencyFull(product.price)}</div>
                                      {cap !== null && (
                                        <div className={`${styles.inventoryCapRow} ${isLow ? styles.inventoryCapLow : ''} ${isSoldOut ? styles.inventoryCapDone : ''}`}>
                                          <div className={styles.inventoryCapBar}>
                                            <div
                                              className={styles.inventoryCapFill}
                                              style={{ width: `${Math.min(100, (sold / cap) * 100)}%` }}
                                            />
                                          </div>
                                          <span className={styles.inventoryCapLabel}>
                                            {isSoldOut ? 'Cap reached' : `${remaining} of ${cap} left`}
                                          </span>
                                        </div>
                                      )}
                                    </div>
                          ...
                          {weekdays.map((day) => (
                            <div key={day.toDateString()} className={styles.slotDateGroup}>
                              ...
                              <div className={styles.slotTimesRow} role="group" aria-label={`Slots for ${formatWeekdayShort(day)}`}>
                                {(scheduleConfig
                                  ? getSlotHours(scheduleConfig, hostService?.duration ?? 60)
                                  : []
                                ).map((hhmm) => {
                                  const { hour, minute } = decodeSlot(hhmm);
                                  const dateStr = day.toISOString().split('T')[0];
                                  const available = scheduleConfig
                                    ? isSlotAvailable(dateStr, hour, minute, hostService?.duration ?? 60, merchant.id, FIXTURE_BOOKINGS, scheduleConfig)
                                    : false;
                                  ...
                                  return (
                                    <m.button
                                      ...
                                      onClick={() => !booked && handleHostSlotSelect(day, hour, minute)}
                                      disabled={booked}
                                      aria-label={`${formatSlotTime(hour, minute)} on ${formatWeekdayShort(day)}${booked ? ' — booked' : ''}`}
        ...
        {isCeremony && (
          ...
          >
            ...
            <div className={styles.ceremonyLogo} aria-hidden="true">
              Trov<span className={styles.ceremonyApostrophe}>é</span>a
            </div>
