...
export default function LedgerPage() {
  ...
  const buyerStats = useMemo(() => {
    const map: Record<string, { count: number; total: number; lastDate: string; preOrders: number; walkIns: number }> = {};
    allReceipts.forEach(r => {
      if (!map[r.buyer_name]) map[r.buyer_name] = { count: 0, total: 0, lastDate: r.created_at, preOrders: 0, walkIns: 0 };
      ...
      if (r.order_type === 'pre-order' || r.order_type === 'preorder') map[r.buyer_name].preOrders++;
      else if (r.order_type === 'walk-in' || r.order_type === 'walkin') map[r.buyer_name].walkIns++;
    });
    return map;
  }, [allReceipts]);
  ...
  const handleResendDelivery = (receiptId: string) => {
    useLedgerStore.getState()._updateReceiptStatus(receiptId, { delivery_status: 'sent' });
    addToast('Delivery link resent to buyer.', 'success');
  };

  const handleMarkManualSent = (receiptId: string) => {
    useLedgerStore.getState()._updateReceiptStatus(receiptId, { delivery_status: 'sent' });
      ...
      <div className={styles.mainContent}>
        ...
        {activeTab === 'delivery' && (
          <div className={styles.deliveryView}>
            {/* To Send — manual_pending queue */}
            {(() => {
              const toSend = allReceipts.filter(r => r.delivery_status === 'manual_pending');
              if (toSend.length === 0) return null;
              return (
                <div className={styles.deliverySection}>
                  <h3 className={styles.fulfilmentTitle}>
                    To Send
                    <span className={styles.deliveryQueueBadge}>{toSend.length}</span>
                  </h3>
                  <p className={styles.deliverySectionHint}>These buyers paid but files were not sent automatically. Send manually, then mark as sent.</p>
                  {toSend.map(receipt => (
                    <div key={receipt.id} className={`${styles.deliveryRow} ${styles.deliveryRowPending}`}>
                      <div className={styles.deliveryInfo}>
                        <div className={styles.deliveryProduct}>{receipt.line_items[0]?.name ?? 'Product'}</div>
                        <div className={styles.deliveryBuyer}>{receipt.buyer_name}</div>
                        <div className={styles.deliveryTime}>{receipt.buyer_email ?? 'No email recorded'}</div>
                      </div>
                      <div className={styles.deliveryStatus}>
                        <button
                          className={styles.resendBtn}
                          onClick={() => handleMarkManualSent(receipt.id)}
                        >
                          Mark Sent ✓
                        </button>
                      </div>
            ...
            <div className={styles.deliverySection}>
              ...
              {allReceipts
                .filter(r => r.delivery_status !== 'manual_pending')
                ...
                .map(receipt => (
                  <div key={receipt.id} className={styles.deliveryRow}>
                    <div className={styles.deliveryInfo}>
                      <div className={styles.deliveryProduct}>{receipt.line_items[0]?.name ?? 'Product'}</div>
                      <div className={styles.deliveryBuyer}>{receipt.buyer_name}</div>
                      <div className={styles.deliveryTime}>{formatDate(receipt.created_at, 'relative')}</div>
                    </div>
                    <div className={styles.deliveryStatus}>
              ...
              {allReceipts.filter(r => r.delivery_status !== 'manual_pending').length === 0 && (
              ...
              <Link key={name} to={`/ledger?buyer=${encodeURIComponent(name)}`} className={styles.clientCard}>
                <div className={styles.clientMain}>
                  ...
                  <div className={styles.clientMeta}>
                    ...
                    {st.isVendor && (stats.preOrders > 0 || stats.walkIns > 0) && ` · ${stats.preOrders} pre-order${stats.preOrders !== 1 ? 's' : ''} · ${stats.walkIns} walk-in${stats.walkIns !== 1 ? 's' : ''}`}
                    {st.isHost && ` · No-show: 0%`}
                  </div>
                </div>
