        ...
        {st.isVendor && (
          <div className={styles.capPill} onClick={(e) => e.stopPropagation()}>
            {inlineEditId === product.id ? (
              ...
              <span onClick={() => onSetInlineEdit(product.id)}>{product.per_window_cap ?? '—'} cap</span>
            )}
          </div>
        )}
...
export default function ArchivePage() {
  ...
  const [watchersDropId, setWatchersDropId] = useState<string | null>(null);
  ...
  const handleBulkAccept = () => {
    ...
    setClaims((prev) =>
      prev.map((c) => ids.includes(c.id) ? { ...c, status: 'accepted', updated_at: new Date().toISOString() } : c)
    );
    setSelectedClaimIds(new Set());
    addToast(`${ids.length} claim${ids.length > 1 ? 's' : ''} accepted.`, 'success');
  };

  const handleBulkDecline = () => {
    const ids = Array.from(selectedClaimIds);
    setClaims((prev) =>
      prev.map((c) => ids.includes(c.id) ? { ...c, status: 'declined', updated_at: new Date().toISOString() } : c)
    );
    setSelectedClaimIds(new Set());
    addToast(`${ids.length} claim${ids.length > 1 ? 's' : ''} declined.`, 'info');
  };
  ...
  return (
    <div className={styles.page}>
      ...
      {st.isCollector && dropsView ? (
        <m.div
          className={styles.dropsView}
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.22 }}
        >
          {drops.length === 0 ? (
            <div className={styles.dropsEmpty}>
              <Info size={20} className={styles.dropsEmptyIcon} />
              <p className={styles.dropsEmptyText}>No drops yet. Schedule your first drop to build excitement before you go live.</p>
            </div>
          ) : (
            drops
              ...
              .map((drop) => {
                const watcherCount = drop.notify_emails.length;
                const statusClass = drop.status === 'live' ? styles.dropStatusLive
                  : drop.status === 'scheduled' ? styles.dropStatusScheduled
                  : drop.status === 'completed' ? styles.dropStatusCompleted
                  : styles.dropStatusDraft;

                return (
                  <div key={drop.id} className={styles.dropCard}>
                    <div className={styles.dropCardMain}>
                      <div className={styles.dropCardLeft}>
                        <span className={`${styles.dropStatusChip} ${statusClass}`}>{drop.status}</span>
                        <p className={styles.dropCardLabel}>{drop.label}</p>
                        <p className={styles.dropCardMeta}>
                          {new Date(drop.scheduled_at).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                          {' · '}
                          {drop.product_ids.length} {drop.product_ids.length === 1 ? 'item' : 'items'}
                        </p>
                      </div>
                      <div className={styles.dropCardRight}>
                        {watcherCount > 0 ? (
                          <button
                            className={styles.watchersBadge}
                            onClick={() => setWatchersDropId(drop.id)}
                            aria-label={`${watcherCount} watcher${watcherCount > 1 ? 's' : ''} for ${drop.label}`}
                          >
                            <Eye size={11} />
                            {watcherCount}
                          </button>
                        ) : (
                          <span className={styles.watchersBadgeEmpty}>
                            <Eye size={11} />
                            0
                          </span>
                        )}
      ...
      {/* ── Watchers Drawer (Collector only) ── */}
      {st.isCollector && (
        <BaseDrawer
          open={watchersDropId !== null}
          onClose={() => setWatchersDropId(null)}
          position="bottom"
          title="Who's Watching"
        >
          {watchersDropId && (() => {
            const watchDrop = drops.find(d => d.id === watchersDropId);
            if (!watchDrop) return null;
            return (
              <div className={styles.watchersBody}>
                <p className={styles.watchersDropName}>{watchDrop.label}</p>
                {watchDrop.notify_emails.length === 0 ? (
                  <p className={styles.watchersEmpty}>No one has subscribed to this drop yet.</p>
                ) : (
                  <div className={styles.watchersList}>
                    {watchDrop.notify_emails.map((email, i) => (
                      <div key={i} className={styles.watcherRow}>
                        <span className={styles.watcherAvatar}>{email[0].toUpperCase()}</span>
                        <span className={styles.watcherEmail}>{email}</span>
                      </div>
                    ))}
                  </div>
                )}
                <p className={styles.watchersHint}>
                  {watchDrop.notify_emails.length} subscriber{watchDrop.notify_emails.length !== 1 ? 's' : ''} will be notified when this drop goes live.
                </p>
              </div>
            );
          })()}
      ...
      {st.isCollector && (
        <BaseDrawer
          ...
          onClose={() => { setClaimReviewProductId(null); setSelectedClaimIds(new Set()); }}
          position="bottom"
          title="Claim Review"
        >
                ...
                {reviewClaims.length === 0 ? (
                  ...
                  <>
                    {selectedClaimIds.size > 0 && (
                      <div className={styles.claimBulkBar}>
                        <span className={styles.claimBulkCount}>{selectedClaimIds.size} selected</span>
                        <div className={styles.claimBulkActions}>
                          <m.button
                      ...
                      <AnimatePresence initial={false}>
                        {reviewClaims.map((claim) => (
                          <m.div
                            key={claim.id}
                            role="listitem"
                            className={`${styles.claimRow} ${claim.status === 'declined' ? styles.claimRowDeclined : ''} ${selectedClaimIds.has(claim.id) ? styles.claimRowSelected : ''}`}
                          ...
                          >
                            {claim.status === 'pending' && (
                              <button
                                className={`${styles.claimCheckbox} ${selectedClaimIds.has(claim.id) ? styles.claimCheckboxChecked : ''}`}
                                onClick={() => toggleClaimSelection(claim.id)}
                                aria-label={`Select claim from ${claim.buyer_name}`}
                                aria-pressed={selectedClaimIds.has(claim.id)}
                              >
