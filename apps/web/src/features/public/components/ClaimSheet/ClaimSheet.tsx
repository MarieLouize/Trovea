...
export default function ClaimSheet({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Stable key per intent — reuse on retries, never regenerate mid-flow
      const idempotencyKey = `claim:${items[0].id}:${(form.buyerPhone || form.buyerEmail).trim()}:${Date.now()}`;
