/**
 * Trovéa — BookingRequestSheet (Phase 3D)
...
import { buildScheduleConfig, getSlotHours, decodeSlot, getAvailableDays, isSlotAvailable } from '@/lib/utils/host';
import { 
...
export default function BookingRequestSheet({
  ...
  const availableDays = useMemo(
    () => getAvailableDays(merchant.id, bookings, schedule),
    [merchant.id, bookings, schedule],
  );
  ...
  const timeSlots = useMemo(
    () => getSlotHours(schedule, service?.duration ?? 60).map(decodeSlot),
            ...
            <div className={styles.timeGrid}>
              {timeSlots.map(({ hour: h, minute: m }) => {
                const isAvailable = selectedDate
                  ? isSlotAvailable(selectedDate, h, m, service?.duration ?? 60, merchant.id, bookings, schedule)
                  : false;
