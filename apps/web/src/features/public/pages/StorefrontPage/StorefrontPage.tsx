/**
 * Trovéa — StorefrontPage (Phase 3 Revision)
...
import {
  ...
  FIXTURE_WINDOWS, FIXTURE_BOOKINGS, FIXTURE_DROPS, FIXTURE_RECEIPTS,
...
import { getNextAvailableDate, getAvailableDays, getSlotHours, decodeSlot, isSlotAvailable, buildScheduleConfig, countSlotsThisWeek } from '@/lib/utils/host';
import {
...
import BookingRequestSheet from '@/features/public/components/BookingRequestSheet';
import MiniCard from '@/features/public/components/MiniCard/MiniCard';
import DigitalCartCheckoutDrawer from '@/features/public/components/DigitalCartCheckoutDrawer/DigitalCartCheckoutDrawer';
import { StorefrontSkeleton } from '@/features/public/components/Skeletons';
...
import { getPendingClaimForProduct } from '@/lib/api/claims.api';
  ...
  return (
    <m.div className={sfStyles.closedInterstitial} {...slideUp}>
      ...
      <p className={sfStyles.closedPowered}>
        Powered by{' '}
        <Link to="/auth" className={sfStyles.closedPoweredLink}>Trovéa</Link>
      </p>
    </m.div>
  );
}
...
function MenuDisplay({
  ...
  capMap?: Record<string, { remaining: number; cap: number }>;
}) {
  const { add, has } = useBasketStore();
        ...
        <div key={cat} className={sfStyles.menuCategory}>
          ...
          <div className={sfStyles.menuItems}>
            {items.map((item) => {
              ...
              const capInfo = capMap[item.id];
              const isSoldOut = capInfo !== undefined && capInfo.remaining <= 0;
              const isLowCap = capInfo !== undefined && capInfo.remaining > 0 && capInfo.remaining <= 3;
              const showCap = capInfo !== undefined && capInfo.remaining <= Math.ceil(capInfo.cap / 2);
              const isAvailable = windowState === 'open' && !isSoldOut;

              return (
                <Link
                  key={item.id}
                  to={`/store/${merchant.handle}/item/${item.id}`}
                  className={`${sfStyles.menuItem} ${isSoldOut ? sfStyles.menuItemSoldOut : ''}`}
                >
                  <img
                  ...
                  <div className={sfStyles.menuItemMain}>
                    ...
                    {isSoldOut ? (
                      <span className={sfStyles.menuItemCapOut}>Cap reached</span>
                    ) : showCap && capInfo ? (
                      <span className={`${sfStyles.menuItemCapSignal} ${isLowCap ? sfStyles.menuItemCapLow : ''}`}>
                        {capInfo.remaining} of {capInfo.cap} remaining
                      </span>
                    ) : null}
                  </div>
                  <div className={sfStyles.menuItemRight}>
                    ...
                    <div className={sfStyles.menuItemActions}>
                      {isAvailable ? (
                        ...
                        <span className={sfStyles.menuItemCapOut} style={{ fontSize: 9 }}>Sold out</span>
                      ) : (
...
function CalendarPreview({
  ...
  const schedule = useMemo(() => buildScheduleConfig(merchant), [merchant]);
  const availableDays = useMemo(
    () => getAvailableDays(merchant.id, FIXTURE_BOOKINGS, schedule),
    [merchant.id, schedule],
  );
  ...
  // Derive time slots from the merchant's schedule (service duration 60 min default)
  const timeSlots = useMemo(
    () => getSlotHours(schedule, 60).map(decodeSlot),
    [schedule],
  );
      ...
      <BaseDrawer open={!!selectedDay} onClose={() => setSelectedDay(null)} title={selectedDay ? formatDate(selectedDay, 'long') : ''}>
        <div className={sfStyles.slotDrawer}>
          ...
          <div className={sfStyles.slotList}>
            {timeSlots.map(({ hour: h, minute: m }) => {
              const isAvailable = selectedDay
                ? isSlotAvailable(selectedDay, h, m, 60, merchant.id, FIXTURE_BOOKINGS, schedule)
                : false;
...
function DigitalProductCard({
  product, handle, merchantId, cardStyle, soldCount = 0
}: {
  product: Product; handle: string; merchantId: string; cardStyle: CardStyle; soldCount?: number;
}) {
  const { add, has } = useBasketStore();
  ...
  const hasEarlyAccess = product.early_access_price != null && product.early_access_cap != null;
  const earlyAccessExhausted = hasEarlyAccess && soldCount >= (product.early_access_cap ?? 0);
  ...
  return (
    <m.div variants={staggerChild} className={sfStyles.digitalCard}>
      ...
      >
        ...
        <div className="sf-card-body">
          ...
          <p className="sf-card-price">
            {isFree ? 'Free' : earlyAccessExhausted
              ? formatCurrencyFull(product.price)
              : product.early_access_price
                ? formatCurrencyFull(product.early_access_price)
                : formatCurrencyFull(product.price)}
          </p>
          {hasEarlyAccess && !earlyAccessExhausted && (
            <p className={sfStyles.earlyAccessCounter}>
              {soldCount} of {product.early_access_cap} early access slots taken
            </p>
          )}
          {hasEarlyAccess && earlyAccessExhausted && (
            <p className={sfStyles.earlyAccessCounter} style={{ color: 'var(--color-fg-ghost)' }}>
              Early access ended · Full price
            </p>
          )}
        </div>
      </Link>
      ...
      <div className={sfStyles.digitalActions}>
        ...
        >
          {isFree ? 'Get Free' : hasEarlyAccess && !earlyAccessExhausted ? `Early Access: ${formatCurrencyFull(product.early_access_price!)}` : 'Buy Now'}
        </Link>
      </div>
    </m.div>
  );
...
export default function StorefrontPage() {
  ...
  const { holds, initFromDB: initHolds, subscribeToChanges: subHolds, unsubscribe: unsubHolds } = useHoldStore();
  const { claims, initFromDB: initClaims, subscribeToChanges: subClaims, unsubscribe: unsubClaims } = useClaimStore();

  useEffect(() => {
    if (merchant.id) {
      initHolds(merchant.id);
      initClaims(merchant.id);
      subHolds();
      subClaims();
    }
    return () => {
  ...
  }, [merchant.id, initHolds, initClaims, subHolds, subClaims, unsubHolds, unsubClaims]);
  ...
  const nextAvailableDate = useMemo(() => {
    ...
    const schedule = buildScheduleConfig(merchant);
    return getNextAvailableDate(merchant.id, FIXTURE_BOOKINGS, schedule);
  }, [storeType, merchant]);

  const hostSlotsThisWeek = useMemo(() => {
    if (storeType !== 'host') return null;
    const schedule = buildScheduleConfig(merchant);
    // Use shortest service duration available; fallback 60 min
    const minDuration = products.length > 0
      ? Math.min(...products.filter(p => p.duration != null).map(p => p.duration!))
      : 60;
    return countSlotsThisWeek(merchant.id, FIXTURE_BOOKINGS, schedule, minDuration);
  }, [storeType, merchant, products]);
  ...
  // Digital Creator: sold count per product (for early access cap display)
  const digitalSoldCounts = useMemo<Record<string, number>>(() => {
    if (storeType !== 'digital_creator') return {};
    const counts: Record<string, number> = {};
    FIXTURE_RECEIPTS.filter(r => r.merchant_id === merchant.id && r.payment_status === 'paid').forEach(r => {
      r.line_items.forEach(li => {
        if (li.product_id) counts[li.product_id] = (counts[li.product_id] ?? 0) + 1;
      });
    });
    return counts;
  }, [storeType, merchant.id]);

  // Vendor: per-item cap remaining for the active window
  const vendorCapMap = useMemo<Record<string, { remaining: number; cap: number }>>(() => {
    if (storeType !== 'vendor' || !activeWindow) return {};
    const winStart = new Date(activeWindow.opens_at).getTime();
    const winEnd = new Date(activeWindow.closes_at).getTime();
    const windowReceipts = FIXTURE_RECEIPTS.filter(r => {
      const t = new Date(r.created_at).getTime();
      return r.merchant_id === merchant.id && t >= winStart && t <= winEnd;
    });
    const soldMap: Record<string, number> = {};
    windowReceipts.forEach(r => {
      r.line_items.forEach(li => {
        if (li.product_id) soldMap[li.product_id] = (soldMap[li.product_id] ?? 0) + (li.quantity ?? 1);
      });
    });
    const result: Record<string, { remaining: number; cap: number }> = {};
    liveProducts.forEach(p => {
      if (p.per_window_cap !== null && p.per_window_cap !== undefined) {
        const sold = soldMap[p.id] ?? 0;
        result[p.id] = { remaining: Math.max(0, p.per_window_cap - sold), cap: p.per_window_cap };
      }
    });
    return result;
  }, [storeType, activeWindow, liveProducts, merchant.id]);
    ...
    >
        ...
        <div className={sfStyles.headerSignalRow}>
          {storeType === 'host' && (
            <>
              ...
              {hostSlotsThisWeek !== null && hostSlotsThisWeek <= 5 && hostSlotsThisWeek > 0 && (
                <div className={sfStyles.slotsUrgencyBadge}>
                  <Clock size={11} />
                  {hostSlotsThisWeek === 1
                    ? '1 slot left this week'
                    : `${hostSlotsThisWeek} slots left this week`}
                </div>
              )}
            </>
          )}
      ...
      {storeType === 'vendor' && (
        <>
          ...
          {windowState === 'open' && activeWindow && (() => {
            const hoursLeft = (new Date(activeWindow.closes_at).getTime() - Date.now()) / 3_600_000;
            if (hoursLeft > 0 && hoursLeft <= 6) {
              return (
                <div className={sfStyles.closingBanner}>
                  <Clock size={13} className={sfStyles.closingBannerIcon} />
                  <span>
                    Window closes in <strong>{Math.ceil(hoursLeft)}h</strong> — order before it's gone
                  </span>
                </div>
              );
            }
          ...
          <MenuDisplay products={liveProducts} merchant={merchant} windowState={windowState} capMap={vendorCapMap} />
          ...
          {featuredProducts.length > 0 && (
            <section className={sfStyles.featuredSection}>
              ...
              <div className={sfStyles.digitalCatalogue}>
                {featuredProducts.map(p => (
                  <DigitalProductCard key={p.id} product={p} handle={handle || merchant.handle} merchantId={merchant.id} cardStyle={cardStyle} soldCount={digitalSoldCounts[p.id] ?? 0} />
                ))}
              </div>
            </section>
          )}
      ...
      <footer className={sfStyles.footer}>
        ...
        <span className={sfStyles.footerBrand}>
          Powered by{' '}
          <Link to="/auth" className={sfStyles.footerBrandLink}>Trovéa</Link>
        </span>
        <button
          ...
          {step === 'success' && (
            <m.div key="success" {...slideUp} className={sfStyles.reportSuccess}>
              ...
              <p className={sfStyles.successText}>
                We've received your report and will investigate. Thank you for keeping Trovéa safe.
              </p>
