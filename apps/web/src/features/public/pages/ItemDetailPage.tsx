/**
 * Trovéa — ItemDetailPage (Phase 3 Revision)
...
import { useClaimStore } from '@/lib/store/claim.store';
...
import ClaimSheet from '@/features/public/components/ClaimSheet';
import HoldSheet from '@/features/public/components/HoldSheet';
...
import { ItemDetailSkeleton } from '@/features/public/components/Skeletons';
...
import { createEnquiry as apiCreateEnquiry } from '@/lib/api/enquiries.api';
...
function Lightbox({
  ...
  }, [current, images.length, onChange, onClose]); // Added missing dependencies
              ...
              <div className={styles.paymentBox}>
                ...
                {bank ? (
                  <div className={styles.bankCard}>
                    ...
                    <p className={bank.account_name}>{bank.account_name}</p>
                  </div>
                ) : (
...
export default function ItemDetailPage() {
  ...
  const { addToast } = useUIStore();
  const { initFromDB: initHolds, getHoldForProduct, addHold, subscribeToChanges: subHolds, unsubscribe: unsubHolds } = useHoldStore();
  const { initFromDB: initClaims, getPendingClaim, subscribeToChanges: subClaims, unsubscribe: unsubClaims } = useClaimStore();
  ...
  }, [handle, item_id, initHolds, initClaims, subHolds, subClaims, unsubHolds, unsubClaims]);
  ...
  const hasPendingClaimLocal = useCallback(
    (productId: string) =>
  ...
  const isClaimed = useMemo(
    () => product ? !!getPendingClaim(product.id) : false,
    [product, getPendingClaim]
  );
  ...
  const onTouchStartMain = (e: React.TouchEvent) => { touchStart.current = e.touches[0].clientX; };
  ...
  return (
    <div className={`sf-themed ${styles.page}`} data-palette={paletteId} data-dark={isDark}>
          ...
          <div className={styles.actions}>
            {isClaimed ? (
              ...
              <>
                ...
                {bagEligible && (
                  <button
                    className={`${styles.actionBtn} ${has(product.id) ? styles.actionBtnBasketAdded : styles.ctaSecondary}`}
              ...
              <>
                <button
                  className={`${styles.actionBtn} ${has(product.id) ? styles.actionBtnBasketAdded : styles.ctaPrimary}`}
                  onClick={handleBasketToggle}
                >
                  {has(product.id)
                    ? <><Check size={15} /> In Bag ({basketItems.length})</>
      ...
      <ClaimSheet
        ...
        hasPendingClaim={hasPendingClaimLocal(product.id)}
