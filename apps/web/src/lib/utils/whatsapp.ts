/**
 * Trovéa — WhatsApp Link Utilities
...
// Studio — Send deliverables (merchant → client)
export function buildDeliverablesLink(params: {
  phone: string;
  buyerName: string;
  projectName: string;
  storeName: string;
  ...
  const { phone, buyerName, projectName, storeName, sealUrl } = params;
  const message = [
    `Hi ${buyerName} 🎉`,
    '',
    `Your ${projectName} project with *${storeName}* is complete!`,
    '',
    `View your Project Brief and deliverables here:`,
    sealUrl,
    '',
    `It's been a pleasure working with you. Please reach out if you have any questions.`,
  ].join('\n');
  return encodeWA(phone, message);
}

// Studio — Request revision (client → merchant)
export function buildRevisionRequestLink(params: {
  phone: string;
  storeName: string;
  projectName: string;
  sealId: string;
}): string {
  const { phone, storeName, projectName, sealId } = params;
  const message = `Hi *${storeName}*, I'd like to request a revision on my *${projectName}* project (Ref: ${sealId}). Here's what I'd like adjusted:\n\n`;
  return encodeWA(phone, message);
}
